...
const Navbar = () => {
  ...
  return (
    ...
    <div className='flex items-center justify-between text-sm py-4 mb-3 border-b border-b-gray-400'>
      <img onClick={()=>{navigate('/')}} className='w-45 cursor-pointer'src={assets.logo}alt=''></img>
      ...
      <div className=' flex items-center gap-4'>
        ...
        <div className={` ${showMenu?'fixed w-full mt-5' :'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all`}>
        <div className='flex items-center justify-between px-5 pу-6'>
        <img className='w-36' src={assets.logo} alt="" />
        <img className='w-7' onClick={()=>setShowMenu(false)} src={assets.cross_icon} alt='' />
        </div>
        <ul className='flex flex-col items-center gap-2 mt-5 px-5 text-1g font-medium'>
